...
def parse_args():
    ...
    parser.add_argument("--house", type=str, default="HOUSE3")
    parser.add_argument("--location", type=str, default="THESS")
...
ARGS = parse_args()
HOUSE = ARGS.house.upper()
LOCATION = ARGS.location.upper()
T = ARGS.hours

########################################################################################################

def _read_hourly_csv(filename, column):
    path = os.path.join(os.path.expanduser(ARGS.data_dir), HOUSE, filename)
    if not os.path.exists(path):
        raise FileNotFoundError(f"Missing file: {path}")
    df = pd.read_csv(path, parse_dates=["datetime"])
    df = df.set_index("datetime").resample("h").mean()
    values = df[column].values[:T]
    if column in {"load_W", "dhw_W", "Q_space_W"}:
...
# Time 
model.T = pyomo.Set(initialize=range(T)) 

# SOLAR RADIATION DATA FROM CSV FILE
ghi_hourly_values = _read_hourly_csv(f"SOLAR_DATA_{LOCATION}.csv", "GHI")
...
# ELECTRICITY DEMAND  
electricity_load = _read_hourly_csv(f"ELECTRICITY_{LOCATION}.csv", "load_W")
...
# DHW DEMAND 
dhw_load = _read_hourly_csv(f"DHW_{LOCATION}.csv", "dhw_W")
...
# SPACE HEAT DEMAND
sph_load = _read_hourly_csv(f"SPACE_HEAT_{LOCATION}.csv", "Q_space_W")
...
# TEMPERATURE DATA
Tamb_series = _read_hourly_csv(f"TEMPERATURES_{LOCATION}.csv", "Tamb_C")
Tcoll_series = _read_hourly_csv(f"TEMPERATURES_{LOCATION}.csv", "Tcoll_C")
...
print(f"ALL DATA LOADED | {HOUSE} - {LOCATION} - {T}")
...
model.heat_store_Thot = pyomo.Param(initialize=60+273.15)   
model.heat_store_Tcold = pyomo.Param(initialize=10+273.15)   
model.heat_store_Tbdg = pyomo.Param(initialize=23+273.15)   
...
model.el_grid_cost = pyomo.Param(initialize=0.20)
model.el_grid_connection_fee = pyomo.Param(initialize=130.0)
model.grid_pco2 = pyomo.Param(initialize=250)

# 3.13
model.U = pyomo.Param(initialize=93)
model.C = pyomo.Param(initialize=27 * 1e6)

# 3.14 
model.c_T = pyomo.Param(initialize=0.1)
model.T_min = pyomo.Param(initialize=19+273.15)  
model.T_max = pyomo.Param(initialize=27+273.15) 

# SUM
model.SOC_el = pyomo.Var(model.T, domain=pyomo.NonNegativeReals)
model.SOC_thermal = pyomo.Var(model.T, domain=pyomo.NonNegativeReals)
...
def pv_electricity_ub(model, t):
    return model.x_el_out_pv[t] <= (model.pv_h_el * model.pv_h_pr * model.I_t[t] * (model.x_el_pv / model.pv_p))
model.constraint_pv_elec = pyomo.Constraint(model.T, rule=pv_electricity_ub)

# 3.4 
def st_heat_ub(model, t):
    st_constraint = max(0.0, pyomo.value(model.st_h * model.I_t[t] - model.st_K * (model.T_coll[t] - model.T_amb[t])))
    return (model.x_sph_out_st[t] + model.x_dhw_out_st[t]) <= st_constraint * model.x_th_st / model.st_p
...
def battery_charge_ub(model, t):
    return model.y_el_in_battery[t] <= model.battery_p_plus * (model.y_el_battery / 3600.0)
model.constraint_battery_ch_ub = pyomo.Constraint(model.T, rule=battery_charge_ub)

# 3.10 
def battery_discharge_ub(model, t):
    return model.y_el_out_battery[t] <= model.battery_p_minus * (model.y_el_battery / 3600.0)
model.constraint_battery_dis_ub = pyomo.Constraint(model.T, rule=battery_discharge_ub)

# 3.10 
def battery_soc_dynamics(model, t):
    if t == 0:
        return (model.SOC_el[0] == model.battery_E * (model.battery_yo * model.y_el_battery) +
                (model.battery_h_plus * model.y_el_in_battery[0] - model.battery_h_minus * model.y_el_out_battery[0]) * model.Dt)
    else:
...
def battery_cycling_limit(model):
    T = len(model.T)
    return (
    sum(model.y_el_out_battery[t] for t in model.T) * model.Dt
...
def heat_store_m2_rule(model):
    return (pi * model.heat_store_U * (((model.heat_store_Thot - model.heat_store_Tcold)/2.0) -
            (model.heat_store_Tbdg - 273.15)) * model.heat_store_F * model.y_h_tank)
...
def building_temperature_rule(model, t):
    if t == 0:
        return (model.T_bdg[t] ==
                model.exponential * model.T_amb[t] +
...
# CUSTOM CUTS
def cut_1(model):
    return model.binary_boiler + model.binary_fc + model.binary_st >= 1
model.cut_1 = pyomo.Constraint(rule=cut_1)

def cut_2(model):
    return model.binary_boiler + model.binary_fc + model.binary_hp + model.binary_st >= 1
model.cut_2 = pyomo.Constraint(rule=cut_2)

def cut_3(model):
    return model.binary_boiler + model.binary_fc + model.binary_hp + model.binary_st <= 3
model.cut_3 = pyomo.Constraint(rule=cut_3)

def cut_4(model):
    return model.binary_tank <= model.binary_fc + model.binary_boiler + model.binary_st
model.cut_4 = pyomo.Constraint(rule=cut_4)

def cut_5(model):
    return model.binary_boiler + model.binary_fc + model.binary_st <= 2 + (1 - model.binary_tank)
model.cut_5 = pyomo.Constraint(rule=cut_5)

def cut_6(model):
    return model.binary_fc + model.binary_pv <= 1 + model.binary_batt
model.cut_6 = pyomo.Constraint(rule=cut_6)
...
# Solar Thermal
st_area = pyomo.value(model.x_th_st) / 650
...
tank_vol_liters = tank_height * np.pi * (0.80 / 2.0) ** 2 * 1000
