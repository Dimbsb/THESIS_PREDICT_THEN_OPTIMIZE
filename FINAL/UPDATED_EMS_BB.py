...
def load_data(location: str, T: int, data_directory: str) -> dict:
    ...
    L_electricity = np.round(_read(f"ELECTRICITY_{location}.csv", "load_W"), 3)
    L_dhw         = np.round(_read(f"DHW_{location}.csv",         "dhw_W"), 3)
    L_sph         = np.round(_read(f"SPACE_HEAT_{location}.csv",  "Q_space_W"), 3)
...
########################################################################################################
    ...
    for t in range(T):
        # 3.10  
        model.addConstr(y_el_in_battery[t] <= battery_p_plus * (y_el_battery /Dt), f"battery_charge_ub{t}") # 3.10.3

        # 3.10
        model.addConstr(y_el_out_battery[t] <= battery_p_minus * (y_el_battery /Dt), f"battery_discharge_ub{t}") # 3.10.4
...
########################################################################################################
    ...
    binary_var_names = {var.VarName for var in binary_vars}
    integer_var = [var.VarName in binary_var_names for var in all_vars]
